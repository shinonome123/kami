...
需要同一四维作用域下 ≥20 条人工批准且排除训练证据的留出终稿。5 语言 × 9 语体 × 4 领域 × N 项目的组合空间中，绝大多数作用域永远攒不够。
...
| 2026-08-26 | 16 | 规则级累计（`src/style-rules.mjs`），根治第 8 节第 4 项「滚动重写导致风格失忆」。蒸馏不再整段重写 instruction，改为对既有规则集提交增量操作（`keep` / `update` / `add` / `retire`）：`ruleId(category, rule)` 给同一条规则文本稳定 id 以便跨轮追溯，`applyRulePatch` 跨轮累加 `evidenceCount` 与 `rounds`，**本轮没被提到的规则不删除**——连续 `DEFAULT_STALE_ROUNDS`（4）轮无人确认才退休并写明原因，退休是标记不是删除。`renderInstruction` 按类别分组、证据多的排前，没有规则时回落到历史散文规范以兼容存量。取样上限同时放宽（正例 30→50），实测蒸馏载荷 30 条约 2k 字符、50 条 3.3k、100 条 6.5k、200 条 13k，50 条对当前模型的上下文与注意力都还宽裕。12 项新测试 | 已实施，待提交 |
| 2026-08-26 | 17 | 三方提示词规则的冲突审查（`src/rule-conflicts.mjs` + `src/conflict-scan.mjs`）。起因：风格规范规则、技能 `additionalRules` 与译者画像 instruction 都往同一份翻译提示词里写文字且互不知情，技能可能带着「句尾用简体」晋升而风格规范写着「句尾统一敬体」，模型只挑一条照做，配对评测只能间接察觉。做法是确定性预筛 + 模型裁决：`ASPECT_PATTERNS` 把规则归到 9 个「方面」，只有共享方面的规则对才成为候选，按方向相反程度排序后至多 12 对交模型。**预筛刻意放宽**——早期版本要求干净的肯定/否定配对，结果漏掉了最重要的一类（中文常把两个方向写在一句里）。同时修正 `ruleDirection` 的否定辖域：「不得使用简体」里的「使用」是被否定的，旧实现判成 mixed 使整套排序失真。裁决门槛很窄（必须指出一个同时遵守就不可能的具体场景，总则与例外不算），模型不可用时只报不可用、绝不把候选当结论。裁决为真后按证据权重衡量（技能规则无证据计数，折算 50 条中等权重；势均力敌交人工），**只有输的是风格规则**才给出退休按钮，走 `retireRule` + `updateStyleProfileRules` 就地改写生效规范而非新建版本；输的是技能或画像一律只报告。触发时机为蒸馏**真的产出新草稿**之后（批次蒸馏与分享反馈采纳两条路径）+ `learning.conflictScanIntervalMinutes` 定时（默认 0）；初版误挂在人工采纳上，那时规则集根本没变，却可能每次采纳都白烧一次模型调用。**实测**（DeepSeek，5 条合成规则）：预筛 5→4 对，模型只判 1 处真冲突，正确放过「统一敬体 vs 战斗吼叫用常体」这类总则与例外，处置正确路由到 review（输的是技能规则）。顺带修掉两个真 bug：`rescheduleConflictScan()` 在 `let conflictScanTimer` 之前调用导致启动 TDZ 崩溃；`src/classifier.mjs` 里 `/\bui\b/` 与 `/\bpv\b/` 的词边界曾被写成字面退格符 0x08，两条描述列规则永远匹配不上（「活动PV脚本」「游戏UI」一律落到 general），而同一正则的其它分支掩盖了故障使测试照样通过。17 项新测试 | 已实施，待提交 |
...
4. ✅ **滚动重写导致风格失忆**（已实施，见施工日志 16）：蒸馏改为对既有规则提交增量操作（keep/update/add/retire），规则有稳定 id、跨轮累加的证据数与轮次；没被本轮提到的规则不删除，连续 4 轮无人确认才退休并留下原因。这是"证据越多规则越多"与"证据越多措辞越换"的分界。
...
6. **作用域爆炸**：风格作用域 5 语言 × 9 语体 × 4 领域 = 180 格，每格需 8 条同域证据，多数格子长期填不满，实际生效的是 `CONTENT_TYPES` 里的硬编码 register。

剩余顺序建议：(2) 的回溯对比（按 `styleProfileId` 聚合 qa_runs）成本很低，可以顺手做掉；(5) 风格证据向量检索与 (6) 作用域稀疏属于长期问题。

配对评测已经让风格改动**可被证伪**，规则级累计又让每一条规则有了独立的身份与支撑度：现在既能证明"这一版整体更好"，也能指出"哪一条规则在起作用、被多少条证据支撑、多久没被确认过"。
