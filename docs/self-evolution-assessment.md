...
| 2026-08-14 | 8 | 批次排比一致性（同批上下文锚定）：新增 `src/batch-verse.mjs`（`verseShape` 短句+长句句式识别、`detectBatchVerse` 严格过半数的批次排比检测、`normalizeBatchReferences` 锚点清洗），服务端按 batchId 自动检测排比模板，客户端顺序翻译时携带本批已定稿译文（最多 3 条）作为风格锚点，提示词/AIQA/自检增加同批句式一致性要求，排比批次走高温度初译；4 项新测试。解决了"同一批诗句各行格式不一致"的问题 | 已实施，待提交 |
